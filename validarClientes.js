...
// - 🚫 REGLA DE NEGOCIO: si la consulta falla o NO devuelve resultados
//   (dato no encontrado, búsqueda vacía, respuesta sin campos útiles),
//   el cupo consumido se revierte automáticamente: el usuario NUNCA
//   pierde créditos por consultas sin éxito.
...
// Cada categoría de consulta tiene su propio par limite/consumidas:
//   'documento'  -> consultasLimite / consultasConsumidas             (DNI, RUC)
//   'telefono'   -> consultasTelefonosLimite / consultasTelefonosConsumidas (telefonía por doc y por número)
//   'dniNombres' -> consultasDniNombresLimite / consultasDniNombresConsumidas (buscar DNI por nombres)
//   'cedula'     -> consultasCedulaLimite / consultasCedulaConsumidas (cédula venezolana y búsqueda por nombre)
// ----------------------------------------------------------------
const CUPO_FIELDS = {
  documento:  { limite: 'consultasLimite',              consumidas: 'consultasConsumidas' },
  telefono:   { limite: 'consultasTelefonosLimite',     consumidas: 'consultasTelefonosConsumidas' },
  dniNombres: { limite: 'consultasDniNombresLimite',    consumidas: 'consultasDniNombresConsumidas' },
  cedula:     { limite: 'consultasCedulaLimite',        consumidas: 'consultasCedulaConsumidas' }
};

async function consumirCupo(userRef, tipo) {
  const campos = CUPO_FIELDS[tipo] || CUPO_FIELDS.documento;
  const limiteField = campos.limite;
  const consumidasField = campos.consumidas;
...
async function ejecutarConsulta({ req, res, tipo, contexto, fetchFn }) {
  ...
  try {
    ...
    // Si el proveedor respondió pero NO devolvió resultados útiles
    // (dato no encontrado, búsqueda vacía), se revierte el cupo
    // consumido para no cobrar la consulta al usuario.
    if (esResultadoVacio(tipo, resultado)) {
      await revertirCupo(req.userRef, tipo);
      logger.info(contexto, 'Consulta sin resultados: cupo no descontado', { uid: req.uid, tipo });
      return res.status(404).json({
        success: false,
        error: 'No se encontraron resultados para el dato ingresado. No se descontó ningún crédito.',
        code: 'NO_RESULTS'
      });
    }
...
function esResultadoVacio(tipo, resultado) {
  if (resultado === null || resultado === undefined) return true;
  // Listas directas (las búsquedas de Masitaprex suelen devolver arrays)
  if (Array.isArray(resultado)) return resultado.length === 0;
  if (typeof resultado === 'object') {
    // Envoltorios con listas internas de resultados
    for (const key of ['resultados', 'results', 'lineas', 'telefonos', 'coincidencias', 'data']) {
      if (Array.isArray(resultado[key])) return resultado[key].length === 0;
    }
    // Objeto plano sin ningún valor útil
    const valores = Object.values(resultado).filter(v => v !== undefined && v !== null && v !== '');
...
router.get('/cupo', requireAuth, async (req, res) => {
  ...
  const consultasDniNombresLimite = data.consultasDniNombresLimite ?? 0;
  const consultasDniNombresConsumidas = Number(data.consultasDniNombresConsumidas || 0);
  const consultasCedulaLimite = data.consultasCedulaLimite ?? 0;
  const consultasCedulaConsumidas = Number(data.consultasCedulaConsumidas || 0);
  ...
  res.json({
    ...
    dniNombres: {
      limite: consultasDniNombresLimite,
      consumidas: consultasDniNombresConsumidas,
      disponibles: consultasDniNombresLimite === -1 ? 'Ilimitados' : Math.max(consultasDniNombresLimite - consultasDniNombresConsumidas, 0)
    },
    cedula: {
      limite: consultasCedulaLimite,
      consumidas: consultasCedulaConsumidas,
      disponibles: consultasCedulaLimite === -1 ? 'Ilimitados' : Math.max(consultasCedulaLimite - consultasCedulaConsumidas, 0)
    }
  });
});
