...
// httpOnly 'user_uid' y 'user_email' creadas en /api/login-success.
// Es indispensable invocarlo desde el frontend al cerrar sesión: el método
// firebase.auth().signOut() solo limpia el estado del SDK de Firebase en el
// navegador, pero no tiene ningún efecto sobre estas cookies httpOnly (el
// frontend no puede leerlas ni borrarlas por sí mismo). Sin este endpoint,
...
// Manejador 404: se ejecuta solo si ninguna ruta ni archivo estático anterior
// coincidió con la petición. Se distingue explícitamente entre:
//  - Rutas de API (/api/*, /v1/*): responden con JSON 404, nunca con HTML.
//    Esto es indispensable para no afectar a developer-api.html ni a ningún
//    otro cliente que haga fetch(...).json() contra la API (por ejemplo,
//    /api/session, /api/user/plan, /api/developer/*, /v1/me): si en vez de
//    JSON recibieran esta página de error en HTML, el parseo fallaría.
//  - Cualquier otra ruta (páginas del sitio no encontradas): se sirve
